...
$id_libro  = $_POST['id_lanzamiento'] ?? 0;
$moderador = trim($_POST['moderador'] ?? '');
$libro     = trim($_POST['titulo_libro'] ?? '');

if (empty($id_libro) || empty($moderador) || empty($libro)) {
    die("Faltan datos del moderador o del libro.");
}

// 🔹 Obtener datos del libro desde la base de datos
$stmt = $pdo->prepare("SELECT * FROM lanzamientos_libros WHERE id = ?");
$stmt->execute([$id_libro]);
$p = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$p) die("Lanzamiento no encontrado.");
...
// 🔹 Token seguro HMAC
$timestamp = time();
$payload = $id_libro . '|' . $moderador . '|' . ($p['no'] ?? '') . '|' . $timestamp;
...
try {
    $stmtIns = $pdo->prepare("INSERT INTO certificados_libro (panel_id, nombre, token, created_by) VALUES (?, ?, ?, ?)");
    $stmtIns->execute([$id_libro, $moderador, $token_b64url, 'pdf_certificado_lanzamiento']);
} catch (PDOException $e) {
    // Ignorar si ya existe
}

// 🔹 URL de verificación y QR
$urlVerificacion = "https://ciesju.udenar.edu.co/app/icons2025/certificar.html?ip3=" . urlencode($id_libro) . "&tok=" . urlencode($token_b64url);
...
$pdf->SetTitle(utf8_decode("Certificación de Lanzamiento ($moderador) - ICONS 2025"));
...
$pdf->SetXY(25, 97);
$pdf->MultiCell(230, 8, utf8_decode("Por el lanzamiento de su libro titulado:"), 0, 'C');
...
// 🔹 Salida PDF
$nombreArchivo = "Certificado Lanzamiento ICON-S2025 (" . utf8_decode($moderador) . ").pdf";
